...
while():
    server = networking.Server_Net()
    client = server.server_Listen(IP,PORT)
    ...
    if (data[0] == 'l'): #login case
         ...
         Players.append(player(userdata[0],client,userdata[1],userdata[1]+userdata[2]))  #create player from userdata and add to playerlist
    ...
    elif (data[0] == 'c'): #chat message case
        data = data[1]
    ...
    elif (data[0] == 'r'): #gamerequest accepted/declined case 
        data = data[1]
